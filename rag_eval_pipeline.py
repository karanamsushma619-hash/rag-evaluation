...
def resolve_metrics(metric_names: List[str]):
    unknown = [name for name in metric_names if name not in METRIC_REGISTRY]
    if unknown:
        raise ValueError(
            f"Unknown metrics: {unknown}. Available metrics: {sorted(METRIC_REGISTRY.keys())}"
        )
...
def evaluate_dataset(
    ...
    selected_metrics = resolve_metrics(metric_names)
